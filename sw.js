const CACHE_NAME='ssp-cache-v35.1';
...
self.addEventListener("activate", (event) => {
  event.waitUntil((async () => {
    const keys = await caches.keys();
    await Promise.all(keys.map((k) => ((k.startsWith("ssp-shell-") || k.startsWith("ssp-cache-") || k.startsWith("ssp-")) && k !== CACHE) ? caches.delete(k) : Promise.resolve()));
