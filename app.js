...
function openAdd(){
  ...
  async function generateDocPdfFromFiles(files){
    ...
    for(let i=0;i<files.length;i++){
      ...
      // compressione forte per velocità + dimensione
    ...
    return { blob, dataUriString, pages: files.length };
  }
    ...
    if(btnDocScan && inDocScan){
      ...
      inDocScan.addEventListener("change", async (e)=>{
        ...
        try{
          ...
          const name = `documento_${todayISO().replace(/-/g,"")}.pdf`;
          const a = document.createElement("a");
