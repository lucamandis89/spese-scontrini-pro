...
function isDetraibileStrict(cat, ocrText){
  ...
  if(c.includes("farmacia") || c.includes("mediche") ||
...
function toast(msg, ms=1600){
  ...
  function badgeHtml(x){
    const b=[];
    if(isCaf(x.category)) b.push('⭐ Detraibile');
    if(x.pdf) b.push('📄 PDF');
    if(x.photo) b.push('📷 Foto');
    const note = String(x.note||"").toLowerCase();
    if((+x.amount||0)===0 || note.includes("da completare")) b.push('📝 Da completare');
    if(x.pendingOCR || note.includes("da analizzare")) b.push('🧠 Da analizzare');
    if(!b.length) return "";
    return `<div class="badges">${b.map(t=>`<span class="badge">${t}</span>`).join("")}</div>`;
  }
  ...
  function calcStats(){
    ...
    const monthItems = all.filter(x=>!isTrashed(x) && x.month===mNow);
    const yearItems  = all.filter(x=>!isTrashed(x) && (x.date||"").startsWith(yNow+"-"));

    const monthTotal = monthItems.reduce((s,x)=>s+(+x.amount||0),0);
    const yearTotal  = yearItems.reduce((s,x)=>s+(+x.amount||0),0);

    const monthCaf   = monthItems.filter(x=>isCaf(x.category)).reduce((s,x)=>s+(+x.amount||0),0);
    const monthNon   = monthTotal - monthCaf;

    const todoCount  = monthItems.filter(x => (+x.amount||0) === 0 || (String(x.note||"").toLowerCase().includes("da completare"))).length;
    const pendingCnt = monthItems.filter(x => !!x.pendingOCR || String(x.note||"").toLowerCase().includes("da analizzare")).length;

    $("#statMonth").textContent = euro(monthTotal);
    $("#statYear").textContent = euro(yearTotal);

    if($("#statMonthCaf")) $("#statMonthCaf").textContent = euro(monthCaf);
    if($("#statMonthNonCaf")) $("#statMonthNonCaf").textContent = euro(monthNon);
    if($("#statTodo")) $("#statTodo").textContent = String(todoCount);
    if($("#statPending")) $("#statPending").textContent = String(pendingCnt);
  ...
  function renderList(){
    ...
    const cafSum = list.filter(x=>isCaf(x.category)).reduce((s,x)=>s+(+x.amount||0),0);
    const cafPct = total ? Math.round((cafSum/total)*100) : 0;
    if($("#sumCount")) $("#sumCount").textContent = String(list.length);
    if($("#sumTotal")) $("#sumTotal").textContent = euro(total);
    if($("#sumCaf")) $("#sumCaf").textContent = euro(cafSum);
    if($("#sumCafPct")) $("#sumCafPct").textContent = `(${cafPct}%)`;
...
function openAdd(){
  ...
  function renderDashboard(){
    ...
    if($("#rSumTotal")) $("#rSumTotal").textContent = euro(t.total);
    if($("#rSumCaf")) $("#rSumCaf").textContent = euro(t.caf);
    if($("#rSumNonCaf")) $("#rSumNonCaf").textContent = euro(t.non);
    if($("#rSumCount")) $("#rSumCount").textContent = String(t.n||0);
  ...
  async 
  // ===========================
  // SAFE: Document Scanner (multi-page PDF from images)
  // ===========================
  async function generateDocPdfFromFiles(files){
    ...
    // UI helpers
    const box = $("#docScanStatus");
    const txt = $("#docScanText");
    const fill = $("#docScanBarFill");
    const setProg = (done,total)=>{
      if(txt) txt.textContent = `PDF: ${done}/${total}`;
      if(fill){
    ...
    let cancelled = false;
    const cancelBtn = $("#btnDocScanCancel");
    const onCancel = ()=>{ cancelled = true; };
    cancelBtn?.addEventListener("click", onCancel, {once:false});
    ...
    for(let i=0;i<files.length;i++){
      ...
      // Compress strongly to speed + reduce size
      const dataUrl = await imageToDataUrl(img, 1240, 1754, 0.60, 0); // fits A4-ish, good quality

      if(i>0) pdf.addPage();

      // Fit image into page keeping ratio
      const info = pdf.getImageProperties(dataUrl);
    ...
    return { pdf, blob, dataUriString, pages: files.length };
  }

  async function saveDocPdfToArchive(dataUriString, pages){
    // Salva come voce in archivio (categoria Documenti) - SAFE: non tocca OCR
    ...
    const item = {
      ...
      amount: 0,
      date: dateVal,
      month: yyyymm(dateVal),
      category: "Documenti",
      note: `Documento PDF (${pages} pagine)`,
      pdf: dataUriString,
      photo: null
    };
    await dbPut(item);
...
function generatePdf(mode, targetMonth){
    if(!window.jspdf || !window.jspdf.jsPDF){
      alert("PDF non disponibile (jsPDF non caricato).");
      return;
    }
...
async function processBatchQueue(){
    ...
    // ===========================
    // Document Scanner (PDF multi-pagina)
    // ===========================
    const btnDocScan = $("#btnDocScan");
    const inDocScan = $("#inDocScan");
    if(btnDocScan && inDocScan){
      btnDocScan.addEventListener("click", ()=>{
        inDocScan.value = "";
        inDocScan.click();
      });

      inDocScan.addEventListener("change", async (e)=>{
        ...
        if(!imgs.length){
          toast("Seleziona solo foto");
          return;
        }

        toast(`Creo PDF (${imgs.length} pagine)...`);
        try{
          const res = await generateDocPdfFromFiles(imgs);
          if(!res) return;

          // Download immediato
          const name = `documento_${todayISO().replaceAll("-","")}.pdf`;
          const a = document.createElement("a");
          a.href = res.dataUriString;
          a.download = name;
          document.body.appendChild(a);
          a.click();
          a.remove();

          // Salva anche in Archivio come "Documenti"
          await saveDocPdfToArchive(res.dataUriString, res.pages);
        }catch(err){
          console.warn(err);
          toast("Errore creazione PDF ⚠️");
        }
      });
    }
