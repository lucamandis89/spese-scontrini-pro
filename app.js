...
$("#addClose").addEventListener("click", closeAdd);
    ...
    async function saveBatchItem(){
      // salva senza chiudere il modal e senza refresh ad ogni elemento
      // usa la stessa logica di onSave ma in versione "silent"
      let amountVal = parseEuro($("#inAmount").value);
      let dateVal = $("#inDate").value;
      const category = $("#inCategory").value;
      const note = ($("#inNote").value || "").trim();
      ...
      // Se manca importo/data prova OCR veloce (se disponibile)
      if((!Number.isFinite(amountVal) || amountVal<=0 || !dateVal) && file && window.__sspReceipt?.handle){
        try{ await window.__sspReceipt.handle(file, "autosave"); }catch(e){}
      }
      ...
      // Se OCR non trova l'importo, salva comunque come "Da completare"
      if(amountVal === 0){
        toast("Importo non trovato: salvato come da completare 📝");
      }
