...
function toast(msg, ms=1600){
  ...
  // MAX COMPRESSION (Batch + OCR faster)
  async function imageToJpegFile(img, maxW=640, maxH=640, quality=0.55, filename="receipt.jpg"){
...
function openAdd(){
  ...
  async function onSave(){
    ...
    if(previewPhoto){
      ...
      const ocrFile = await imageToJpegFile(img, 640, 640, 0.55, "receipt.jpg");
      const imgSmall = await fileToImage(ocrFile);
      photo = await imageToDataUrl(imgSmall, 0, null, 1.0, 0);
    }
