...
function openAdd(){
  ...
  async 
  // ===========================
  // SAFE: Scanner Documenti (PDF multi-pagina da foto)
  // ===========================
  async function __sspGenerateMultiPagePdfFromImages(files, onProgress, isCancelled){
    ...
    for(let i=0;i<files.length;i++){
      ...
      // compressione forte ma leggibile (veloce e leggera)
      const dataUrl = await imageToDataUrl(img, 1240, 1754, 0.60, 0);
  ...
  async function __sspSavePdfDocumentToArchive(pdfDataUri, pages){
 ...
 <Worksheet ss:Name="Spese">
  <Table>
   ...
   ${rows}
    ...
    // ===========================
    // SAFE: Document Scanner (PDF multi-pagina)
    // ===========================
    const btnDocScan = $("#btnDocScan");
    const inDocScan = $("#inDocScan");
    const docBox = $("#docScanStatus");
    const docTxt = $("#docScanText");
    const docFill = $("#docScanBarFill");
    const docCancel = $("#btnDocScanCancel");
    let __docCancelled = false;

    const setDocProg = (done,total)=>{
      if(docTxt) docTxt.textContent = `PDF: ${done}/${total}`;
      if(docFill){
        const pct = total ? Math.round((done/total)*100) : 0;
    ...
    if(btnDocScan && inDocScan){
      ...
      inDocScan.addEventListener("change", async (e)=>{
        ...
        try{
          ...
          setDocProg(0, imgs.length);

          const pdf = await __sspGenerateMultiPagePdfFromImages(
            imgs,
            (d,t)=>setDocProg(d,t),
            ()=>__docCancelled
          );
          ...
          // save in archive
          await __sspSavePdfDocumentToArchive(dataUri, imgs.length);
          ...
          console.warn(err);
          if(docBox) docBox.style.display="none";
          if(String(err?.message||"").includes("jsPDF_not_loaded")){
            toast("PDF non disponibile ⚠️");
          }else{
