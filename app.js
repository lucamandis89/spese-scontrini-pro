...
function isDetraibileStrict(cat, ocrText){
  ...
  if(c.includes("visite") || c.includes("ticket") || c.includes("dentist") || c.includes("odont") || c.includes("ottic")) return true;
  if(c.includes("sport") && c.includes("rag")) return true;
...
(() => {
  ...
  const CATEGORIES = [
    "Alimentari","Benzina","Casa","Bollette","Farmacia / Mediche","Visite / Ticket","Dentista","Ottico","Bambini",
    "Sport ragazzi","Animali","Lavoro","Ristorante / Bar","Viaggi","Scuola","Assicurazioni","Altro"
  ];

  const CAF_CATEGORIES = new Set([
    "Farmacia / Mediche", "Visite / Ticket", "Dentista", "Ottico", "Sport ragazzi",
    "Scuola", "Bambini", "Assicurazioni", "Casa", "Bollette"
  ]);
...
function renderPreset730(){
  const bar = $("#preset730Bar");
  if(!bar) return;
  bar.innerHTML = PRESET_730.map(c=>(
    `<button type="button" class="btn btn-preset btn-sm" data-cat="${escapeHtml(c)}">${escapeHtml(c)}</button>`
  )).join("");
...
// ---------------- BACKUP ----------------
  async function exportBackup(){
    const payload = {
      ...
      version:6,
      exportedAt:new Date().toISOString(),
      meta:{ dbName: APP.dbName, store: APP.store, total: (all||[]).length, active: activeList(all).length, trashed: trashList(all).length },
      settings:{
        isPro: settings.isPro,
  ...
  async function importBackup(file){
    try{
      ...
      const doReplace = confirm("Import backup:
OK = UNISCI (non cancella)
Annulla = scegli sovrascrittura");
      let replaceAll = false;
      if(!doReplace){
        replaceAll = confirm("Sovrascrivere TUTTO?
Cancella le spese attuali e importa il backup.");
        if(!replaceAll) return;
      }

      if(replaceAll){
        await dbClear();
        // reset in-memory list so merge logic below works
        all = [];
      }
      ...
      toast(`Import OK ✅ (+${added}${updated?`, aggiornati ${updated}`:""} )`);
      await refresh();
    }catch{
