...
$("#addClose").addEventListener("click", closeAdd);
    ...
    let batchRunning = false;
      window.__sspBatchRunning = false;
    ...
    async function saveBatchCurrent(file){
      // Salvataggio diretto per batch: evita conflitti con autosave ritardato
      let amountVal = parseEuro($("#inAmount").value);
      let dateVal = $("#inDate").value;
      const category = $("#inCategory").value;
      const note = ($("#inNote").value || "").trim();

      // Se OCR non ha trovato dati validi, non bloccare il batch: salta in modo sicuro
      if(!Number.isFinite(amountVal) || amountVal<=0 || !dateVal){
        return false;
      }
      ...
      scheduleAutoBackup();
            ...
            // Refresh UNA volta a fine batch
      try{ await refresh(); }catch(e){}

// Reset campi minimi per la prossima scansione
      previewPhoto = null;
      scanImg = null;
      setPhotoPreview(null);

      // Mantieni categoria, svuota importo/note
      $("#inAmount").value = "";
      $("#inNote").value = "";
...
unifiedFileInput.addEventListener('change', async (e) => {
    ...
    window.addEventListener('ssp:ocr-filled', ()=>{
      try{
        if(window.__sspBatchRunning) return;
