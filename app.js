...
(() => {
  ...
  const APP = {
    ...
    dbVersion: 1,  // rimane 1, la migrazione del campo avviene via refresh
    store: "expenses",
...
settings.reminders = settings.reminders || []; // per promemoria
saveSettings();
...
async function importBackup(file){
  ...
  function wire(){
    ...
    if (onboardingModal) {
      let step = 1;
      const slides = [1,2,3].map(i => document.getElementById(`onboardingSlide${i}`));
      ...
      function updateStep() {
        slides.forEach((s, idx) => s.style.display = (idx+1 === step) ? 'block' : 'none');
        prevBtn.disabled = (step === 1);
        counter.innerText = `${step}/3`;
        if (step === 3) {
      ...
      prevBtn.onclick = () => { if (step > 1) { step--; updateStep(); } };
      nextBtn.onclick = () => { if (step < 3) { step++; updateStep(); } };
      const finish = () => {
  ...
  (async function start(){
    ...
    // Mostra onboarding se non già visto
    if (!localStorage.getItem('onboarding_completed')) {
      setTimeout(() => showModal('#onboardingModal'), 300);
    }
  })();

// --- NUOVO: Gestione tasto indietro ---
let modalStack = [];
...
function popModal() {
  if (modalStack.length > 0) {
    ...
    hideModal(lastModal);
    history.back(); // torna allo stato precedente
  } else {
...
// Ascolta il tasto indietro del browser
...
// Sovrascrivi showModal e hideModal per gestire lo stack
...
// --- NUOVO: Tema dinamico (segue sistema) ---
function applySystemTheme() {
  const prefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
  const savedTheme = localStorage.getItem('theme'); // 'light' o 'dark'
  
  if (!savedTheme) {
    // Se l'utente non ha scelto esplicitamente, usa il sistema
...
// Sincronizza toggle con tema salvato
if (themeToggle) {
  ...
  if (savedTheme === 'light') {
    ...
    // Nessuna preferenza: usa sistema
    applySystemTheme();
    // Il toggle rimane false, ma il tema effettivo potrebbe essere light
    // Qui potresti anche impostare il toggle in base al sistema, ma è meglio lasciarlo neutro
  }
}

})();
// NOTA: il resto del file (moduli esterni come PDF import, multi batch, simple mode, hotfix) 
// va mantenuto invariato dopo questa chiusura. Se il tuo file originale aveva codice dopo l'IIFE,
// lascialo così com'è.
