...
function toast(msg, ms=1600){
  ...
  function markPendingOCR(item){
    item.pendingOCR = true;
    if(!item.note || !String(item.note).trim()) item.note = "Da analizzare";
    if(!Number.isFinite(item.amount) || item.amount < 0) item.amount = 0;
    return item;
  }

  function pendingCount(list){
    return (list || []).filter(x => x && x.pendingOCR).length;
  }

  function updatePendingButton(){
    const btn = $("#btnAnalyzePending");
    if(!btn) return;
    const allItems = window.__sspAllItems || [];
    const n = pendingCount(allItems);
    btn.style.display = n > 0 ? "inline-flex" : "none";
    btn.textContent = `🧠 Analizza ora (${n})`;
  }
  ...
  async function imageToJpegFile(img, maxW=512, maxH=512, quality=0.45, filename="receipt.jpg"){
...
function haptic(ms=10){ try{ navigator.vibrate && navigator.vibrate(ms); }catch{} }
    ...
    el.innerHTML = list.map(x=>`
      <div class="item" data-open="${escapeHtml(x.id)}">
        ...
        <div class="meta">
          ...
          <div class="sub">${escapeHtml(x.date)} • ${escapeHtml(x.category)} ${x.pendingOCR?`<span class="pending-badge">Da analizzare</span>`:""}</div>
        </div>
  ...
  async function refresh(){
    ...
    // expose for pending OCR button
    window.__sspAllItems = all;
    updatePendingButton();
...
function openAdd(){
  ...
  async function onSave(){
    ...
    if((!Number.isFinite(amountVal) || amountVal<=0 || !dateVal) && file && window.__sspReceipt?.handle){
      try{ await window.__sspReceipt.handle(file, "autosave"); }
      catch(err){
        console.warn("OCR autosave failed", err);
        window.__sspForcePendingOCR = true;
        // non bloccare il salvataggio: verrà marcato "Da analizzare"
      }
    }
    ...
    const forcePending = !!window.__sspForcePendingOCR;

    // Se OCR è fallito (offline/API), salva comunque come "Da analizzare"
    if((!Number.isFinite(amountVal) || amountVal<=0) && !forcePending){ toast("Importo non valido"); haptic(18); return; }
    if(!dateVal && !forcePending){ toast("Seleziona una data"); haptic(18); return; }

    let amount = Number.isFinite(amountVal) ? amountVal : 0;
    let date = dateVal || todayISO();

    if(forcePending){
      if(!Number.isFinite(amount) || amount < 0) amount = 0;
      window.__sspForcePendingOCR = false;
    }
    ...
    if(previewPhoto){
      ...
      photo = await imageToDataUrl(img, (settings.photoMaxSide||512), (settings.photoMaxSide||512), (settings.photoJpegQuality||0.45), 0);
    }
    ...
    // Marca pending se OCR fallito/offline
    if(forcePending){
      markPendingOCR(item);
    }
...
// ---------------- BACKUP ----------------
  ...
  function wire(){
    ...
    // Pending OCR - Analizza ora (batch)
    const btnAnalyze = $("#btnAnalyzePending");
    if(btnAnalyze){
      btnAnalyze.addEventListener("click", async ()=>{
        const items = (window.__sspAllItems || []);
        const pending = items.filter(x=>x && x.pendingOCR && x.photo);
        if(!pending.length){ toast("Niente da analizzare ✅"); updatePendingButton(); return; }

        // porta l'utente nella schermata add per mostrare progress
        showPage("home");
        let done = 0;
        for(const it of pending){
          try{
            // prepara file compresso da dataURL
            const img = await dataUrlToImage(it.photo);
            const f = await imageToJpegFile(img, 512, 512, 0.45, "receipt.jpg");

            // esegui OCR (riempie i campi)
            window.__sspForcePendingOCR = false;
            try{ await window.__sspReceipt?.handle?.(f, "pending"); }catch(e){ throw e; }

            // leggi campi
            const a = parseEuro($("#inAmount").value);
            const d = $("#inDate").value;
            const c = $("#inCategory").value;
            const n = ($("#inNote").value||"").trim();

            if(Number.isFinite(a) && a>0) it.amount = a;
            if(d) it.date = d;
            it.month = yyyymm(it.date || d || todayISO());
            if(c) it.category = c;
            if(n) it.note = n;

            // se abbiamo almeno importo e data, chiudi pending
            if(Number.isFinite(it.amount) && it.amount>0 && (it.date||d)){
              it.pendingOCR = false;
            }else{
              it.pendingOCR = true;
            }
            ...
            console.warn("pending analyze failed", err);
            // resta pending
            it.pendingOCR = true;
            try{ await dbPut(it); }catch(_){}
          }

          done++;
          toast(`Analisi: ${done}/${pending.length}`);
          updatePendingButton();
    ...
    async function handleUnifiedFile(file){
      ...
      try {
        ...
        try {
          ...
          console.warn("OCR select failed", err);
          window.__sspForcePendingOCR = true;
          toast(isNetworkError(err) ? "Offline / OCR non disponibile: salva come 'Da analizzare' 📴" : "OCR non riuscito: salva come 'Da analizzare' ⚠️");
        }
...
unifiedFileInput.addEventListener('change', async (e) => {
    ...
    $("#btnOpenScanner").addEventListener("click", async ()=>{
      ...
      try{ await window.__sspReceipt.handle(file, "manual"); }
      catch(err){
        console.warn("OCR manual failed", err);
        window.__sspForcePendingOCR = true;
        toast(isNetworkError(err) ? "Offline / OCR non disponibile: salva come 'Da analizzare' 📴" : "OCR non riuscito: salva come 'Da analizzare' ⚠️");
      }
    });
