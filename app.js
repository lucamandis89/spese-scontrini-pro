// === app.js - Versione finale e testata ===

// Funzioni globali (per compatibilità con chiamate esterne)
...
// Event listener globali per OCR copy/download (funzionano sempre)
...
<DTASOF>${new Date().toISOString().replace(/[-:]/g, '').split('.')[0]}
  ...
  async function openScanner() {
    console.log("openScanner chiamata, scanImg =", scanImg);
  ...
  function initEventListeners() {
    ...
    // Pulsante OCR (offline) - chiama manualmente l'OCR
    ...
    // Migliora foto
    $("#btnEnhancePhoto")?.addEventListener("click", openScanner);
    ...
    $("#camAutoShotManual")?.addEventListener("click", () => {
      captureAutoFrame();
    });
  ...
  (async function start() {
    ...
    await openDB();
    await refresh();
    initEventListeners(); // Attacca tutti i listener dopo che il DOM è pronto
    showPage("home");
